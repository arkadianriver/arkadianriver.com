...
Currently, the only two parent categories are `topics` and `works`.

Advanced:

{:.nobullet}
- If you list additional categories (sub-categories) and you want a
  more sophisticated breadcrumb than Home > Topics, create a parent post
  for that sub-category to act just as a page would, but with a permalink.
...
{:.nobullet}
- For topics within that sub-category, list the same categories as the parent
  but without the permalink, resulting in:
  `/topics/my-great-vacation/topic-title.html`

{:.nobullet}
- For that subcategory parent page, you'll want to make an index of the topics
  in the sub-category and maybe even back and next links.
